	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(filesize==0) { //첨부파일 없을경우 
				this.result = this.mu.n_modify(nidx, n_yn, n_subject, n_writer, n_content);
			
			}else {  //첨부파일 있을경우 
				this.result = this.mu.n_modify_withf(nidx, n_yn, n_subject, n_writer, n_content, n_filenm, request);
			}
